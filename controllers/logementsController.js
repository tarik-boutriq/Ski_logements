const { PrismaClient } = require('@prisma/client');
const prisma = new PrismaClient();

const getLogements = async (req, res) => {
    let results = await prisma.logement.findMany();
    
    if (req.query.station) {
        results = results.filter(logement => logement.station === req.query.station);
    }
    if (req.query.capacite) {
        results = results.filter(logement => logement.capacite >= parseInt(req.query.capacite));
    }
    if (req.query.prix_par_nuit) {
        results = results.filter(logement => logement.prix_par_nuit <= parseInt(req.query.prix_par_nuit));
    }
    /*if (req.query.date_debut && req.query.date_fin) {
        const start = new Date(req.query.date_debut);
        const end = new Date(req.query.date_fin);
        
        const reservations = await prisma.reservation.findMany();

        results = results.filter(logement => {
            const conflit = reservations.find(r => r.logement_id === logement.custom_id && start < new Date(r.end) && end > new Date(r.start));
            return !conflit;
        });
    }*/
    
    if (results.length != 0) {
        res.json(results);
    } else {
        res.status(404).json({ message: "Aucun logement !!" });
    }
};

const createLogement = async (req, res) => {
  const { nom, station, prix_par_nuit, capacite } = req.body;

  const logementsExistants = await prisma.logement.findMany({
      orderBy: { custom_id: 'desc' },
      take: 1
  });
  
  let newId;
  
  if (logementsExistants.length > 0) {
      newId = logementsExistants[0].custom_id + 1;
  } else {
      newId = 1;
  }

  const newLogement = {
    custom_id: newId,
    nom,
    station,
    prix_par_nuit: parseInt(prix_par_nuit),
    capacite: parseInt(capacite)
  };

  await prisma.logement.create({ data: newLogement });
  res.status(201).json(newLogement);
};

const getLogementById = async (req, res) => {
  const Id = parseInt(req.params.id);
  
  const logement = await prisma.logement.findUnique({
      where: { custom_id: Id }
  });

  if (logement) {
    res.json(logement);
  } else {
    res.status(404).json({ message: "Logement non trouvé !" });
  }
};

const updateLogement = async (req, res) => {
  const Id = parseInt(req.params.id);

  const logement = await prisma.logement.findUnique({
      where: { custom_id: Id }
  });

  if (!logement) {
    return res.status(404).json({ message: "Logement non trouvé !" });
  } else {
    const body = req.body || {};
    const logementModifie = {
      nom: body.nom || logement.nom,
      station: body.station || logement.station,
      prix_par_nuit: body.prix_par_nuit ? parseInt(body.prix_par_nuit) : logement.prix_par_nuit,
      capacite: body.capacite ? parseInt(body.capacite) : logement.capacite
    };
    
    await prisma.logement.update({
        where: { custom_id: Id },
        data: logementModifie
    });
    
    res.status(200).json({ custom_id: Id, ...logementModifie });
  }
};

const deleteLogement = async (req, res) => {
  const Id = parseInt(req.params.id);
  
  await prisma.logement.delete({
      where: { custom_id: Id }
  });
  
  res.status(204).send();
};

module.exports = {
  getLogements,
  createLogement,
  getLogementById,
  updateLogement,
  deleteLogement,
};